...
%MRS3: probably should explicitly give the colors along with the rhos.
	\caption{Compressibility factors $(Z)$ along isochores for \textit{n}-alkanes. Most force fields are accurate at saturation $(Z \approx 0)$ but deviate strongly at higher pressures. Different symbol shapes correspond to the various force fields. Densities are distinguished by color, increase vertically, and are labeled such that $\rho^{\rm IC}_0 < \rho^{\rm IC}_1 < \rho^{\rm IC}_2 < \rho^{\rm IC}_3 < \rho^{\rm IC}_4$. Panels a) to d) correspond to ethane, propane, \textit{n}-butane, and \textit{n}-octane, respectively.
