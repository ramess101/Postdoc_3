...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\end{enumerate}
	    \item Optimal $\lambda$ for high pressures
	\end{enumerate}
	\item Future Work
	\item Conclusions	
\end{enumerate}
...
\begin{enumerate}
	...
	\item TraPPE and Potoff have parameters for each compound studies, while TraPPE-2 only has parameters for ethane, and TAMie and ErrExp-6 only have parameters for the \textit{n}-alkanes
...
\begin{enumerate}
	...
	\item We perform this analysis for CH3, CH2, CH, and C sequentially and independently. In other words, the Markov Chain only samples a 2-dimensional space.
\end{enumerate}
...
\begin{enumerate}
	...
	\item To determine the optimal value of lambda for compressed liquid pressures, we redefine the posterior using only saturated liquid density and compressed liquid pressures  
\end{enumerate}
